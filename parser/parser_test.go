...
func TestParserScenarios(t *testing.T) {
	...
	control { if speed < max_speed AND temp < max_temp : state = true else : state = false }
		...
		{
			name: "greater than operator parses",
			input: `device motor : PLC {
	watchdog: 500ms
	cycle: 100ms
	boot { state = false }
	inputs { temp = val(sensor(A1), min: -40, max: 150) }
	outputs { motor_relay = actuator(D0) }
	safety { if temp > max_temp : state = false }
	failsafe { state = false output(motor_relay, off) }
	control { if temp < max_temp AND temp < max_temp : state = true else : state = false }
}`,
			assertion: func(t *testing.T, p *Parser, program *ast.Program) {
				stmt := program.Device.Safety.Statements[0].(*ast.IfStatement)
				if stmt.Operator != ">" {
		...
		{
			name: "less than or equal operator parses",
			input: `device motor : PLC {
	watchdog: 500ms
	cycle: 100ms
	boot { state = false }
	...
	control { if temp < max_temp AND temp < max_temp : state = true else : state = false }
		...
		{
			name: "not equal operator parses",
			input: `device motor : PLC {
	watchdog: 500ms
	cycle: 100ms
	boot { state = false }
	...
	control { if temp < max_temp AND temp < max_temp : state = true else : state = false }
		...
		{
			name: "equal equal operator parses",
			input: `device motor : PLC {
	watchdog: 500ms
	cycle: 100ms
	boot { state = false }
